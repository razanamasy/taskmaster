...
def timer(list_proc_data, key, curr_pid, running_table, mutex_proc_dict):
    ...
    print(timestamp('INFO') + "end of stoptime", flush=True)
    ...
    if list_proc_data[key].pid == curr_pid: #Si pas eu d'autre start ou restart entre temp
        ...
        if list_proc_data[key].running[0] == False: #On a reussit a kill
            print(timestamp('WARN') + "the process has already stopped", flush=True)
            return
        else:
            print(timestamp('WARN') + "Stop failed Need to sigkill it", flush=True)
...
def main(list_proc_data, key, clients, running_table, mutex_proc_dict, thread_list):
    print(timestamp('INFO') + f"THIS PROCESS ENTER IN STOP {key}", flush=True)
    process = list_proc_data[key]
    ...
    if process.stopping[0] == True:
        print(timestamp('WARN') + "Process : " + key + " already in a stopping process", flush=True)
    ...
    if process.backlog[0] == True and process.backoff_starting[0] == False: #en backoff
        ...
        print(timestamp('INFO') + "Old state of :" + key + " BACKOFF, stopping the start process", flush=True)
...
#        return "Process : " + key + " already in a starting process"
    if process.stopped[0] == True or process.exited[0] == True or process.fatal[0] == True:
        print(timestamp('INFO') + "Already stopped Process : " + key + " stopped", flush=True)
    ...
    try:
        ...
        stop_timer.start()
        print(timestamp('INFO') + "Stopping process : " + key, flush=True)
        return "Stopping process : " + key
    except OSError as e:
        print(timestamp('ERRO') +  "key :" + str(e), flush=True)
        return "key :" + str(e)
